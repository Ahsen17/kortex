...
from .exception import ParallelProcessError
...
class WorkerProcess(mp.Process):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            setproctitle.setproctitle(f"kortex-worker-{self._process_id}")
        except ImportError:
            pass
    ...
    async def _worker_main(self) -> None:
        ...
        # Create broker
        broker: ParallelBrokerABC = ParallelBroker(**self._broker_config)
        ...
        try:
            ...
            with contextlib.suppress(TimeoutError):
                await asyncio.wait_for(pool.stop(), timeout=self._config.graceful_timeout)
...
class ParallelProcess:
    ...
    def __init__(
        ...
        self._broker_config = broker_config or {"host": "localhost", "port": 6379}
    ...
    def start(self) -> None:
        ...
        if self.processes:
            raise ParallelProcessError("Processes already running")
    ...
    def stop(self, graceful: bool = True) -> None:
        ...
        if graceful:
            ...
            for _i, process in enumerate(self.processes):
                try:
                    process.join(timeout=self._config.graceful_timeout)
                    if process.is_alive():
                        process.terminate()
